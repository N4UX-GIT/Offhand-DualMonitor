...
| Automated addon baseline | PASS | All 19 Lua suites and all five addon manifest validations passed on 2026-09-23. |
...
| Confirmed open runtime defects | NONE AFTER SOURCE FIX / LIVE GAP | The Restore Window absent-topology defect is fixed in current source with focused regression coverage. Its exact live reproduction remains required before release acceptance. |
...
| v2.0.0 Forever Edit Mode / `CompactUnitFrame` taint report | FIXED IN CURRENT SOURCE (automated) / GAP (exact live regression) | The 2026-09-19 report occurred before commit `886bc3a` (2026-09-20). v2.0.0 replaced Blizzard `EditModeUtil`, directly repositioned Forever action/party frames, attached Edit Mode handlers and auto-selected a layout; those paths could both misplace protected HUD elements and taint Blizzard's party-frame refresh on Edit Mode exit. Current Forever code leaves `EditModeUtil`, protected HUD anchors and layout selection Blizzard-owned. Regression asserts those exclusions, but the reported open/close Edit Mode flow with party frames visible must still be repeated live. |
| v2.0.0 Forever CharacterFrame / `TextStatusBar` taint report | FIXED IN CURRENT SOURCE (automated) / GAP (exact live regression) | The PlayerFrame health-text failure occurs while Blizzard closes CharacterFrame, but v2.0.0 had already called `PlayerFrame:SetUserPlaced(false)`, directly reanchored PlayerFrame and installed a `SetPoint` hook. Current Forever excludes PlayerFrame from login placement writes, HUD anchoring, repair hooks and generic Canvas movement. Focused regressions assert those ownership boundaries; repeat CharacterFrame open/close with status text enabled in the live client. |
...
| Two-display vertical stack photo (`3818x2104`) | SUPPORTED IN CURRENT MODEL / GAP (exact hardware) | User clarification identifies one upper landscape screen and one lower pad/laptop arranged vertically. That is within current Companion scope: select both Windows displays, choose the upper display as Mainhand and retain their exact unequal rectangles. The photo's `Mixed Portrait + Landscape` banner proves it was instead using the legacy combined-resolution heuristic; its manually selected 50% stacked split cannot accurately describe unequal display heights and likely explains the world/UI extending across both screens. Re-test with Companion exact topology; retain this hardware combination as a validation gap until its Windows display map and live result are available. |
...
| External publication | GITHUB BETA 2 PUBLISHED / CURSEFORGE USER-MANAGED | GitHub pre-release `2.1.2-beta.2` was published on 2026-09-23 with addon, Companion, standalone EXE, complete bundle and checksum assets. GitHub-reported asset digests matched the local files. The user manages any CurseForge upload. Do not merge, promote to stable, modify the live website or submit antivirus reports without further approval. |
