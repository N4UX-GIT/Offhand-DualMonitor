...
| Forever Escape and native menu placement after taint hardening | INTENTIONAL CHANGE / LIVE GAP | Offhand no longer intercepts `CloseAllWindows` or mutates native panel metadata on Forever. Escape can close map, character, bags or another Blizzard panel before a subsequent press opens the Game Menu; saved Offhand geometry remains for reopening. Game Menu visibility is expected on the accepted 4000x2560 topology but requires live confirmation. Edit Mode still opens through Blizzard, but its native top controls may land in mixed-height void; use Companion **Restore Window**, edit, then respan until a taint-safe in-span access path is proven. |
