local _, Offhand = ...
if GetLocale() ~= "zhCN" then return end
local L = Offhand.L

-- Settings and guided setup
L["CARD_SCALE_ALIGNMENT"] = "游戏对齐与界面缩放"
L["GLOBAL_SCALE_HELP"] = "设置共用界面缩放；插件未自行设置缩放时会继承此值。"
L["CARD_PERSISTENCE"] = "Offhand 显示器窗口"
L["CARD_RECOVERY"] = "恢复与预览"
L["CARD_BEZEL"] = "边框补偿（高级）"
L["PREVIEW_DUAL"] = "在单个屏幕上预览双屏布局"
L["GATHER_UI"] = "找回屏幕外窗口"
L["GATHER_COMBAT"] = "请脱离战斗后再找回窗口。"
L["GATHER_RESULT"] = "已找回 %d 个屏幕外窗口，可见窗口未移动。"
L["PROFILE_NEW_NAME"] = "新配置名称"
L["PROFILE_ACTIVE"] = "（当前）"
L["PROFILE_COPY_INTO_CURRENT"] = "复制到当前配置"
L["PROFILE_COPY_CONFIRM"] = "将“%s”的设置替换为“%s”的副本？此操作无法撤销。"
L["PROFILE_DELETE_CONFIRM"] = "删除配置“%s”？此操作无法撤销。"
L["PROFILE_RESET_CONFIRM"] = "将配置“%s”恢复默认？此操作无法撤销。"
L["SETTINGS_AUTOSAVE"] = "更改立即更新当前配置，WoW 会在退出登录或 /reload 时写入磁盘。"
L["SETTINGS_SAVED_LIVE"] = "更改自动生效"
L["BTN_CLOSE"] = "关闭"
L["BTN_REAPPLY"] = "重新应用布局"
L["BTN_NEXT"] = "下一步"
L["BTN_BACK"] = "上一步"
L["BTN_FINISH"] = "完成设置"
L["RAW_MOUSE_INPUT"] = "Offhand 跨屏时使用原始鼠标输入"
L["RAW_MOUSE_INPUT_TIP_TITLE"] = "可靠的镜头移动"
L["RAW_MOUSE_INPUT_TIP_DESC"] = "防止隐藏光标碰到不同高度窗口的边缘并阻止镜头垂直移动。退出跨屏布局后，Offhand 会恢复先前的原始鼠标设置。"
L["FOREVER_SINGLE_SCREEN_LAYOUT_TEXT"] = "已保存的工作区显示器当前不可用。Forever 会保护动作条和战斗框体，因此 Offhand 需要你点击一次，才能切换到整洁的单屏布局。\n\nOffhand 会记住受保护的 HUD 布局，并在显示器恢复后提示还原。"
L["FOREVER_USE_MODERN"] = "使用现代布局"
L["FOREVER_KEEP_OFFHAND"] = "保留 Offhand"
L["FOREVER_RESTORE_LAYOUT_TEXT"] = "已保存的显示器布局再次可用。Forever 需要你点击一次，才能更改受保护的 HUD 布局。\n\n现在还原 Offhand 布局吗？"
L["FOREVER_RESTORE_OFFHAND"] = "还原 Offhand"
L["FOREVER_KEEP_MODERN"] = "保留现代布局"
L["WIZARD_RECOMMENDATION_APPLIED"] = "已应用推荐设置"
L["WIZARD_CHECK_RECOMMENDATION"] = "继续检查显示器布局、对齐和界面大小。"
L["BTN_SETTINGS"] = "全部设置"
L["STEP_PROGRESS"] = "第 %d 步，共 4 步"
L["STEP_1_HELP"] = "将 WoW 设为窗口模式，再点击 Companion 的 Span Now。推荐值基于游戏窗口大小，请在后续步骤中按实际显示器检查。"
L["STEP_2_HELP"] = "选择游戏显示器及其宽高比，另一个将成为 Offhand 显示器。"
L["STEP_3_HELP"] = "将红色参考线对齐显示器边界；Mainhand 区域过高或过低时调整底部偏移。"
L["STEP_4_HELP"] = "选择舒适的界面大小，更改立即生效。游戏和 Offhand 区域适合显示器后即可完成设置。"
L["HELP_SETUP_TITLE"] = "设置双显示器"
L["HELP_SETUP_BODY"] = "1. 将 WoW 设为窗口模式。\n2. 点击 Companion 的 Span Now，再打开设置向导。\n3. 检查布局、分界线、底部对齐和界面缩放。"
L["HELP_COMPANION_TITLE"] = "为什么 Forever 需要 Companion v2.0+"
L["HELP_COMPANION_BODY"] = "WoW 插件无法调整 Windows 游戏窗口大小，也无法将其无边框跨越多个显示器。Forever 测试版在下次启动时也可能无法可靠恢复插件保存数据。Companion 会跨越所选显示器、移除窗口边框，并在 WoW 关闭后保存最新有效的 Offhand 状态，供下次冷启动恢复。目前，可靠的 Forever 冷启动需要 Companion v2.0.0 或更高版本。"
L["HELP_PANELS_TITLE"] = "使用 Offhand 显示器"
L["HELP_PANELS_BODY"] = "拖动支持窗口的标题栏即可移至任意显示器。启用独立面板可同时打开多个窗口。在世界地图上按住 Ctrl 滚动鼠标滚轮可调整大小。"
L["HELP_RECOVERY_TITLE"] = "找回窗口或返回单屏"
L["HELP_RECOVERY_BODY"] = "使用“找回屏幕外窗口”恢复不可见窗口。返回单屏时关闭双屏模式，再点击 Companion 的 Restore Window（Ctrl+Alt+R）。"
L["HELP_EDIT_MODE_TITLE"] = "在编辑模式中定位战斗框体"
L["HELP_EDIT_MODE_BODY"] = "Forever 由暴雪编辑模式管理动作条和战斗框体，以避免受保护界面错误。请在脱战后打开编辑模式，将动作条以及姿态、宠物、小队和团队框体移动到主显示器，将布局保存为 Offhand，并在编辑模式中选择它。Offhand 不会自动切换布局。"
L["EDIT_MODE_LAYOUT_MISSING"] = "Forever 使用暴雪编辑模式管理动作条和战斗框体。请在脱战后将它们放到主显示器，并将布局保存为“Offhand”。"

L["HELP_OVERVIEW_TITLE"] = "Offhand Setup & Recovery Handbook"
L["HELP_OVERVIEW_BODY"] = "Follow the numbered sections once from top to bottom. Use the buttons to reopen the Wizard, gather ordinary off-screen windows, or get Companion. On Forever, Blizzard Edit Mode owns protected combat UI."
L["HELP_COLD_LAUNCH_TITLE"] = "Everyday launch, shutdown and cold-start check"
L["HELP_COLD_LAUNCH_BODY"] = "Start Companion before WoW and leave it running. Let spanning finish before opening panels. Exit WoW normally before closing Companion so it can capture the newest Forever recovery state. After setup changes, perform one full exit and relaunch to verify the window, workspace panels and Edit Mode manager."
L["HELP_WELCOME_TITLE"] = "Welcome message and setup status"
L["HELP_WELCOME_BODY"] = "Either welcome button acknowledges the introduction; completing the final Wizard step records setup completion separately. Both states are account-wide and included in Forever recovery. If an older build repeats the message, update Offhand, click either action once, then /reload."

-- Core & General Strings
L["ADDON_TITLE"] = "Offhand：多显示器设置"
L["ADDON_DESC"] = "将双屏划分为主屏 3D 游戏区域与副屏工作区。"
L["CMD_HELP_TITLE"] = "Offhand 命令"
L["LAYOUT_REAPPLIED"] = "已重新应用布局！"
L["CONFIG_SAVED"] = "设置已保存！欢迎使用 Offhand。"

-- Options Dashboard Header & Tabs
L["OPTIONS_TITLE"] = "OFFHAND 多屏工作区"
L["TAB_DISPLAY"] = "显示与游戏区域"
L["TAB_WORKSPACE"] = "Offhand 显示器与地图"
L["TAB_THEMES"] = "主题与颜色"
L["BTN_AUTO_WIZARD"] = "自动设置向导"
L["BTN_AUTO_WIZARD_TIP_TITLE"] = "显示校准向导"
L["BTN_AUTO_WIZARD_TIP_DESC"] = "打开向导以检测分辨率并校准显示器分界线。"

-- Tab 1: Display & Viewport
L["CARD_LAYOUT_PRESETS"] = "1. 显示器布局预设"
L["CARD_LAYOUT_PRESETS_DESC"] = "选择分别显示 3D 游戏世界和 2D Offhand 工作区的显示器。"
L["PRESET_PL_LR"] = "左侧竖屏 + 右侧游戏"
L["PRESET_PL_LR_TIP_TITLE"] = "左侧竖屏 + 右侧游戏"
L["PRESET_PL_LR_TIP_DESC"] = "适用于左侧竖向 Offhand 显示器与右侧横向游戏显示器。"

L["PRESET_GL_PR"] = "左侧游戏 + 右侧竖屏"
L["PRESET_GL_PR_TIP_TITLE"] = "左侧游戏 + 右侧竖屏"
L["PRESET_GL_PR_TIP_DESC"] = "适用于左侧游戏显示器与右侧竖向 Offhand 显示器。"

L["PRESET_DUAL_LANDSCAPE"] = "双横屏（50/50）"
L["PRESET_DUAL_LANDSCAPE_TIP_TITLE"] = "并排双横屏"
L["PRESET_DUAL_LANDSCAPE_TIP_DESC"] = "将游戏窗口平均分配到两个相同的横向显示器。"

L["BTN_1CLICK_AUTOCONFIG"] = "一键自动设置"
L["BTN_1CLICK_AUTOCONFIG_TIP_TITLE"] = "自动检测硬件"
L["BTN_1CLICK_AUTOCONFIG_TIP_DESC"] = "检测分辨率并应用推荐的分界比例、方向和宽高比。"
L["EXACT_TOPOLOGY_LOCKED"] = "Companion 管理显示器布局。请在其中更改显示器或 Mainhand，然后重新跨屏。"
L["COMPANION_TOPOLOGY_SUMMARY"] = "Companion 已启用：游戏 %dx%d | 工作区 %dx%d"

L["CARD_VIEWPORT_AR"] = "2. 3D 区域宽高比与位置"
L["AR_16_9"] = "16:9 标准宽屏"
L["AR_16_9_TIP_TITLE"] = "16:9 宽高比"
L["AR_16_9_TIP_DESC"] = "锁定 16:9 视角，必要时添加上下黑边，避免横向拉伸。"

L["AR_21_9"] = "21:9 超宽屏"
L["AR_21_9_TIP_TITLE"] = "21:9 宽高比"
L["AR_21_9_TIP_DESC"] = "锁定 21:9 超宽视角，扩大视野。"

L["AR_FILL"] = "填充窗口高度"
L["AR_FILL_TIP_TITLE"] = "匹配窗口高度（填充模式）"
L["AR_FILL_TIP_DESC"] = "按设定高度百分比纵向填充 3D 区域，不保留上下黑边。"

L["ALIGN_LABEL"] = "Mainhand 对齐："
L["ALIGN_CENTER"] = "居中"
L["ALIGN_CENTER_TIP_TITLE"] = "居中对齐"
L["ALIGN_CENTER_TIP_DESC"] = "将游戏视图置于 Mainhand 区域中央。"

L["ALIGN_LEFT"] = "左侧"
L["ALIGN_LEFT_TIP_TITLE"] = "左对齐"
L["ALIGN_LEFT_TIP_DESC"] = "将游戏视图对齐 Mainhand 区域左边缘。"

L["ALIGN_RIGHT"] = "右侧"
L["ALIGN_RIGHT_TIP_TITLE"] = "右对齐"
L["ALIGN_RIGHT_TIP_DESC"] = "将游戏视图对齐 Mainhand 区域右边缘。"

L["SLIDER_GAME_HEIGHT"] = "游戏高度比例："
L["SLIDER_GAME_HEIGHT_TIP_TITLE"] = "Mainhand 高度"
L["SLIDER_GAME_HEIGHT_TIP_DESC"] = "填充模式下 Mainhand 占窗口高度的百分比。"

L["LABEL_BOTTOM_OFFSET"] = "游戏底部偏移（像素）："
L["LABEL_BOTTOM_OFFSET_TIP_TITLE"] = "底部边距"
L["LABEL_BOTTOM_OFFSET_TIP_DESC"] = "将游戏底部上移指定像素，为任务栏或其他界面留出空间。"

L["CARD_SEAM_CALIBRATION"] = "3. 显示器分界与参考线"
L["SLIDER_SEAM_WIDTH"] = "Offhand 显示器宽度（%）："
L["SLIDER_SEAM_WIDTH_TIP_TITLE"] = "显示器分界位置"
L["SLIDER_SEAM_WIDTH_TIP_DESC"] = "以跨屏总宽度百分比设置 Offhand 与 Mainhand 的分界位置。"

L["BTN_SEAM_MINUS"] = "- 1%"
L["BTN_SEAM_MINUS_TIP_TITLE"] = "分界线左移"
L["BTN_SEAM_MINUS_TIP_DESC"] = "将分界线向左移动 1%。"

L["BTN_SEAM_PLUS"] = "+ 1%"
L["BTN_SEAM_PLUS_TIP_TITLE"] = "分界线右移"
L["BTN_SEAM_PLUS_TIP_DESC"] = "将分界线向右移动 1%。"

L["BTN_LASER_TOGGLE"] = "切换参考线"
L["BTN_LASER_TOGGLE_TIP_TITLE"] = "红色边框参考线"
L["BTN_LASER_TOGGLE_TIP_DESC"] = "显示或隐藏红色垂直参考线，调整分界滑块使其与实际显示器边框对齐。"

L["SLIDER_BEZEL_GAP"] = "边框间隙校正："
L["SLIDER_BEZEL_GAP_TIP_TITLE"] = "边框补偿"
L["SLIDER_BEZEL_GAP_TIP_DESC"] = "创建空白区域以补偿显示器之间的实体边框，避免视觉错位。"

-- Tab 2: Offhand Monitor & Map
L["CARD_WORKSPACE_MGMT"] = "1. Offhand 窗口管理"
L["CHECK_CANVAS_ENABLED"] = "启用 Offhand 工作区"
L["CHECK_CANVAS_ENABLED_TIP_TITLE"] = "Offhand 工作区"
L["CHECK_CANVAS_ENABLED_TIP_DESC"] = "启用用于放置窗口、地图、角色面板和背包的 Offhand 背景。"

L["CHECK_ESC_PERSIST"] = "按 Esc 时保留 Offhand 窗口"
L["CHECK_ESC_PERSIST_TIP_TITLE"] = "独立 Offhand 窗口"
L["CHECK_ESC_PERSIST_TIP_DESC"] = "Esc 不关闭 Offhand 停靠窗口，仅取消目标或打开游戏菜单。"

L["CHECK_ALLOW_DRAG"] = "允许跨屏拖动窗口"
L["CHECK_ALLOW_DRAG_TIP_TITLE"] = "跨屏拖动"
L["CHECK_ALLOW_DRAG_TIP_DESC"] = "允许在 Mainhand 与 Offhand 之间拖动角色、法术书、背包等支持的窗口。"

L["CHECK_SEAM_REDIRECT"] = "自动将窗口放到 Offhand"
L["CHECK_SEAM_REDIRECT_TIP_TITLE"] = "自动窗口重定向"
L["CHECK_SEAM_REDIRECT_TIP_DESC"] = "打开暴雪角色、法术书、任务日志等面板时自动移至 Offhand。"

L["CHECK_SEAM_SNAP"] = "吸附到边缘与分界线"
L["CHECK_SEAM_SNAP_TIP_TITLE"] = "边缘吸附"
L["CHECK_SEAM_SNAP_TIP_DESC"] = "拖动窗口时吸附到 Offhand 边缘与显示器分界线。"

L["SLIDER_HUD_SCALE"] = "全局界面缩放："
L["SLIDER_HUD_SCALE_TIP_TITLE"] = "全局界面缩放"
L["SLIDER_HUD_SCALE_TIP_DESC"] = "缩放动作条、单位框体和对话框。多屏建议使用 56% 至 70%。"

-- Tab 2: Minimap Section
L["CARD_MINIMAP_CONFIG"] = "2. 小地图设置"
L["CHECK_DOCK_MINIMAP"] = "将小地图停靠到 Offhand"
L["CHECK_DOCK_MINIMAP_TIP_TITLE"] = "Offhand 小地图"
L["CHECK_DOCK_MINIMAP_TIP_DESC"] = "将小地图从游戏显示器移至 Offhand 顶部。"

L["SLIDER_MINIMAP_SCALE"] = "小地图缩放倍数："
L["SLIDER_MINIMAP_SCALE_TIP_TITLE"] = "小地图大小"
L["SLIDER_MINIMAP_SCALE_TIP_DESC"] = "调整 Offhand 小地图大小（0.6x 至 2.0x）。"

L["CHECK_LOCK_MINIMAP"] = "锁定 Offhand 小地图位置"
L["CHECK_LOCK_MINIMAP_TIP_TITLE"] = "锁定小地图"
L["CHECK_LOCK_MINIMAP_TIP_DESC"] = "防止意外拖动 Offhand 上的小地图。"

L["BTN_RESET_MINIMAP"] = "重置 Offhand 小地图位置"
L["BTN_RESET_MINIMAP_TIP_TITLE"] = "重置小地图"
L["BTN_RESET_MINIMAP_TIP_DESC"] = "将小地图位置、显示层级和布局重置到 Offhand 顶部中央。"

-- Tab 2: Bags Section
L["CARD_BAG_MGMT"] = "3. 背包管理"
L["CHECK_DOCK_BAGS"] = "将所有背包停靠到 Offhand"
L["CHECK_DOCK_BAGS_TIP_TITLE"] = "Offhand 背包停靠"
L["CHECK_DOCK_BAGS_TIP_DESC"] = "自动将打开的背包放到 Offhand，保持游戏视野清晰。"

L["CHECK_VERTICAL_BAGS"] = "强制背包纵向排列"
L["CHECK_VERTICAL_BAGS_TIP_TITLE"] = "背包纵向列"
L["CHECK_VERTICAL_BAGS_TIP_DESC"] = "在副屏将背包纵向堆叠，而不是横向展开。"

L["SLIDER_BAG_SCALE"] = "背包缩放倍数："
L["SLIDER_BAG_SCALE_TIP_TITLE"] = "背包窗口大小"
L["SLIDER_BAG_SCALE_TIP_DESC"] = "调整 Offhand 背包缩放（0.6x 至 1.5x）。"

-- Tab 3: Themes & Colors
L["CARD_THEMES"] = "1. 视觉主题预设"
L["THEME_CLASSIC"] = "经典魔兽"
L["THEME_CLASSIC_DESC"] = "金边与石质背景的魔兽风格"
L["THEME_CLASSIC_TIP_TITLE"] = "经典魔兽主题"
L["THEME_CLASSIC_TIP_DESC"] = "使用暴雪原版对话框美术与石质纹理。"

L["THEME_SLATE"] = "暴雪板岩"
L["THEME_SLATE_DESC"] = "深灰对话框与银色边框"
L["THEME_SLATE_TIP_TITLE"] = "暴雪板岩主题"
L["THEME_SLATE_TIP_DESC"] = "深炭灰主题搭配精致银色边框。"

L["THEME_TINKER"] = "侏儒工匠"
L["THEME_TINKER_DESC"] = "黄铜边框与亮青色工程装饰"
L["THEME_TINKER_TIP_TITLE"] = "侏儒工匠主题"
L["THEME_TINKER_TIP_DESC"] = "灵感源自侏儒工程蓝图，采用古铜与电光青色。"

L["THEME_OBSIDIAN"] = "暗黑曜石"
L["THEME_OBSIDIAN_DESC"] = "现代深色主题搭配低调石质边框"
L["THEME_OBSIDIAN_TIP_TITLE"] = "暗黑曜石主题"
L["THEME_OBSIDIAN_TIP_DESC"] = "简洁现代的深色主题。"

L["THEME_PITCH_BLACK"] = "纯黑（OLED）"
L["THEME_PITCH_BLACK_DESC"] = "适用于 OLED 屏幕的纯黑"
L["THEME_PITCH_BLACK_TIP_TITLE"] = "纯黑 OLED 主题"
L["THEME_PITCH_BLACK_TIP_DESC"] = "纯黑背景可关闭 OLED 像素以获得最高对比度。"

L["CARD_TRIM_COLOR"] = "2. 边框与强调色"
L["TRIM_GOLD"] = "暴雪金"
L["TRIM_BRASS"] = "机械黄铜"
L["TRIM_CYAN"] = "护目镜青"
L["TRIM_SILVER"] = "锡银"
L["TRIM_BRONZE"] = "暖铜"
L["TRIM_EMERALD"] = "翡翠绿"
L["TRIM_CRIMSON"] = "绯红"

L["CARD_CANVAS_BACKGROUND"] = "3. 工作区背景颜色与不透明度"
L["CANVAS_STONE"] = "经典石材"
L["CANVAS_TINKER"] = "工匠板岩"
L["CANVAS_CHARCOAL"] = "炭灰板岩"
L["CANVAS_WARM_NIGHT"] = "暖夜"
L["CANVAS_NAVY"] = "午夜蓝"
L["CANVAS_BLACK"] = "纯黑"
L["SLIDER_CANVAS_OPACITY"] = "背景不透明度："
L["SLIDER_CANVAS_OPACITY_TIP_TITLE"] = "背景透明度"
L["SLIDER_CANVAS_OPACITY_TIP_DESC"] = "将 Offhand 背景不透明度设为 0% 至 100%。"

-- Wizard Dialog Strings & Tooltips
L["WIZARD_TITLE"] = "OFFHAND 自动配置向导"
L["WIZARD_CARD1_TITLE"] = "1. 显示拓扑与一键自动设置"
L["WIZARD_DETECTED_PREFIX"] = "检测到显示器："
L["WIZARD_RECOM_PREFIX"] = "推荐设置："
L["WIZARD_BTN_AUTOCONFIG"] = "一键自动配置并应用（推荐）"
L["WIZARD_BTN_AUTOCONFIG_TIP_TITLE"] = "一键自动校准"
L["WIZARD_BTN_AUTOCONFIG_TIP_DESC"] = "根据检测到的屏幕分辨率快速校准双屏设置。一键应用分界比例、3D 视口与方向。"
L["WIZARD_STATUS_READY"] = "点击上方按钮自动检测分辨率并配置分界线、方向与视口。"
L["WIZARD_STATUS_APPLIED"] = "[已应用] 已自动完成适用于 %s 的配置"

L["WIZARD_CARD2_TITLE"] = "2. 显示器方向与 3D 视口"
L["WIZARD_LABEL_LAYOUT"] = "显示器布局预设："
L["WIZARD_LABEL_AR"] = "Mainhand 宽高比："

L["WIZARD_CARD3_TITLE"] = "3. 物理显示器分界对齐"
L["WIZARD_SEAM_INSTRUCTION"] = "调整红色激光参考线，使其与两台显示器之间的实体边框对齐："
L["WIZARD_LABEL_SEAM"] = "Offhand 宽度："
L["WIZARD_BTN_LASER_SHOW"] = "显示激光线"
L["WIZARD_BTN_LASER_HIDE"] = "隐藏激光线"
L["WIZARD_PRESET_SEAM_36"] = "1440/4000 分界（36%）"
L["WIZARD_PRESET_SEAM_36_TIP_TITLE"] = "1440p 竖屏 + 4K 横屏"
L["WIZARD_PRESET_SEAM_36_TIP_DESC"] = "配置 36% 分界比例，专为 1440x2560 竖屏搭配 2560x1440 横屏量身定制。"

L["WIZARD_PRESET_SEAM_50"] = "等分（50%）"
L["WIZARD_PRESET_SEAM_50_TIP_TITLE"] = "50% 等分布局"
L["WIZARD_PRESET_SEAM_50_TIP_DESC"] = "在两台等宽显示器之间平均分割画面。"

L["WIZARD_PRESET_SEAM_55"] = "非对称分割（55%）"
L["WIZARD_PRESET_SEAM_55_TIP_TITLE"] = "55% 非对称分割"
L["WIZARD_PRESET_SEAM_55_TIP_DESC"] = "将总屏幕宽度的 55% 分配给左侧显示器，45% 分配给右侧。"

L["WIZARD_CARD4_TITLE"] = "4. 全局界面缩放与校准"
L["WIZARD_UI_SCALE_INSTRUCTION"] = "调整用户界面的整体尺寸，以适合您的视距："
L["WIZARD_LABEL_UI_SCALE"] = "全局界面缩放："
L["WIZARD_UI_SCALE_TIP_TITLE"] = "全局界面缩放"
L["WIZARD_UI_SCALE_TIP_DESC"] = "缩放所有动作条、单位框体和对话框。建议选择紧凑尺寸（56% 至 70%）以保持游戏画面开阔。"

L["WIZARD_PRESET_SCALE_56"] = "紧凑（56%）"
L["WIZARD_PRESET_SCALE_56_TIP_TITLE"] = "紧凑界面（56%）"
L["WIZARD_PRESET_SCALE_56_TIP_DESC"] = "极致精简比例，为 3D 游戏世界留出最大的可视空间。"

L["WIZARD_PRESET_SCALE_65"] = "均衡（65%）"
L["WIZARD_PRESET_SCALE_65_TIP_TITLE"] = "均衡界面（65%）"
L["WIZARD_PRESET_SCALE_65_TIP_DESC"] = "兼顾清晰文字可读性与宽敞视野的平衡比例。"

L["WIZARD_PRESET_SCALE_70"] = "标准（70%）"
L["WIZARD_PRESET_SCALE_70_TIP_TITLE"] = "标准界面（70%）"
L["WIZARD_PRESET_SCALE_70_TIP_DESC"] = "Offhand 默认标准比例，适合正常桌面视距下的 1440p 与 4K 显示器。"

L["WIZARD_PRESET_SCALE_100"] = "原始大小（100%）"
L["WIZARD_PRESET_SCALE_100_TIP_TITLE"] = "无缩放界面（100%）"
L["WIZARD_PRESET_SCALE_100_TIP_DESC"] = "暴雪原生标准 100% 尺寸，不作任何缩减。"

L["WIZARD_PRESET_SCALE_85"] = "宽松舒适（85%）"
L["WIZARD_PRESET_SCALE_85_TIP_TITLE"] = "宽松界面（85%）"
L["WIZARD_PRESET_SCALE_85_TIP_DESC"] = "专为偏好更大字体与图标的玩家设计的清晰比例。"

L["WIZARD_BTN_ADVANCED"] = "高级设置（/offhand）"
L["WIZARD_BTN_ADVANCED_TIP_TITLE"] = "高级设置"
L["WIZARD_BTN_ADVANCED_TIP_DESC"] = "关闭向导并打开包含完整控制选项的 3 标签页控制面板。"

L["WIZARD_BTN_FINISH"] = "完成设置"
L["WIZARD_BTN_FINISH_TIP_TITLE"] = "完成校准"
L["WIZARD_BTN_FINISH_TIP_DESC"] = "保存配置，标记初始设置已完成，并应用新的多屏布局。"

L["WIZARD_BTN_CLOSE"] = "关闭向导"
L["WIZARD_BTN_CLOSE_TIP_TITLE"] = "关闭向导"
L["WIZARD_BTN_CLOSE_TIP_DESC"] = "关闭设置向导且不保存新的更改。"

-- Wizard — Inline Display Text (non-button)
L["WIZARD_SEAM_VAL_FMT"] = "分界：%.1f%%"
L["WIZARD_SCALE_VAL_FMT"] = "缩放：%.0f%%"
L["WIZARD_SEAM_EDIT_TIP_TITLE"] = "手动输入分界宽度"
L["WIZARD_SEAM_EDIT_TIP_DESC"] = "输入百分比（如 36%）或比率（如 0.36）后按 Enter 键应用分界位置。"
L["WIZARD_SCALE_EDIT_TIP_TITLE"] = "手动输入界面缩放"
L["WIZARD_SCALE_EDIT_TIP_DESC"] = "输入百分比（如 70%）或小数（如 0.70）后按 Enter 键应用全局缩放。"
L["WIZARD_BTN_SCALE_DOWN_TIP_TITLE"] = "缩小"
L["WIZARD_BTN_SCALE_DOWN_TIP_DESC"] = "将全局界面缩放降低 1%。"
L["WIZARD_BTN_SCALE_UP_TIP_TITLE"] = "放大"
L["WIZARD_BTN_SCALE_UP_TIP_DESC"] = "将全局界面缩放提高 1%。"
L["WIZARD_BTN_SCALE_RESET"] = "重置（70%）"
L["WIZARD_BTN_SCALE_RESET_TIP_TITLE"] = "重置界面缩放"
L["WIZARD_BTN_SCALE_RESET_TIP_DESC"] = "将全局界面缩放重置为推荐的标准默认值 70%。"

-- Options Panel — Tab Labels & General Buttons
L["BTN_APPLY_LAYOUT"] = "应用布局"
L["BTN_APPLY_LAYOUT_TIP_TITLE"] = "应用布局"
L["BTN_APPLY_LAYOUT_TIP_DESC"] = "立即强制所有视口、分界线和界面元素更新为当前设置。"
L["BTN_SAVE_CLOSE"] = "关闭"
L["BTN_SAVE_CLOSE_TIP_TITLE"] = "关闭"
L["BTN_SAVE_CLOSE_TIP_DESC"] = "保存所有设置并关闭配置面板。"
L["LABEL_BOTTOM_OFFSET_SHORT"] = "游戏底部偏移（像素）："

-- Options Panel — Slider Step & Edit Tooltips
L["SLIDER_STEP_UP_TIP_TITLE"] = "增加"
L["SLIDER_STEP_UP_TIP_DESC_FMT"] = "将数值增加 %s。"
L["SLIDER_STEP_DOWN_TIP_TITLE"] = "减少"
L["SLIDER_STEP_DOWN_TIP_DESC_FMT"] = "将数值减少 %s。"
L["SLIDER_EDITBOX_TIP_TITLE"] = "手动输入数值"
L["SLIDER_EDITBOX_TIP_DESC"] = "点击输入精确数值或百分比后按 Enter 键。"

-- Options Panel — World Map Section
L["BTN_MAP_AUTOFIT"] = "自动贴合"
L["BTN_MAP_AUTOFIT_TIP_TITLE"] = "自动适配世界地图"
L["BTN_MAP_AUTOFIT_TIP_DESC"] = "自动缩放世界地图以精准贴合 Offhand 显示器的宽度。"
L["BTN_MAP_100_TIP_TITLE"] = "100% 地图缩放"
L["BTN_MAP_100_TIP_DESC"] = "将世界地图设为标准 100% 缩放。"
L["BTN_MAP_150_TIP_TITLE"] = "150% 地图缩放"
L["BTN_MAP_150_TIP_DESC"] = "将世界地图设为 150% 缩放以查看更宏大详细的视图。"
L["BTN_MAP_200_TIP_TITLE"] = "200% 地图缩放"
L["BTN_MAP_200_TIP_DESC"] = "将世界地图设为 200% 缩放。"
L["BTN_MAP_250_TIP_TITLE"] = "250% 地图缩放"
L["BTN_MAP_250_TIP_DESC"] = "将世界地图设为 250% 缩放，填满副屏的大部分区域。"

-- Options Panel — Canvas & Trim Color Buttons
L["BTN_CANVAS_TONE_TIP_TITLE_FMT"] = "%s 色调"
L["BTN_CANVAS_TONE_TIP_DESC_FMT"] = "将 Offhand 显示器背景色调设为 %s。"
L["BTN_CANVAS_CUSTOM"] = "自定义颜色..."
L["BTN_CANVAS_CUSTOM_ACTIVE_FMT"] = "自定义 (#%02x%02x%02x)"
L["BTN_CANVAS_CUSTOM_TIP_TITLE"] = "自定义画布颜色"
L["BTN_CANVAS_CUSTOM_TIP_DESC"] = "打开调色盘，为副屏 Offhand 选择自定义背景颜色与不透明度。"
L["BTN_CANVAS_PREVIEW_TIP_TITLE"] = "实时背景预览"
L["BTN_CANVAS_PREVIEW_TIP_DESC"] = "点击此色块打开颜色选择器并自定义副屏背景颜色。"
L["BTN_TRIM_ACCENT_TIP_TITLE_FMT"] = "%s 强调色"
L["BTN_TRIM_ACCENT_TIP_DESC_FMT"] = "对对话框边框、滑块手柄及界面框架应用 %s 高亮着色。"
L["BTN_TRIM_CUSTOM"] = "自定义强调色..."
L["BTN_TRIM_CUSTOM_ACTIVE_FMT"] = "自定义 (#%02x%02x%02x)"
L["BTN_TRIM_CUSTOM_TIP_TITLE"] = "自定义强调色"
L["BTN_TRIM_CUSTOM_TIP_DESC"] = "打开颜色选择器，为边框、标题栏及滑块手柄自选强调色。"

-- Options Panel — Guide Link & Autoconfig Print
L["BTN_GUIDE_LINK"] = "查看窗口跨屏指南"
L["BTN_GUIDE_LINK_TIP_TITLE"] = "窗口跨屏指南"
L["BTN_GUIDE_LINK_TIP_DESC"] = "查看跨多台物理显示器运行 World of Warcraft 的操作说明与批处理脚本。"
L["MSG_AUTOCONFIG_APPLIED"] = "|cff00ff00一键自动配置已应用：|r %s"
L["MSG_AUTOCONFIG_DETAILS"] = "预设：|cffffd100%s|r | 分界：|cffffd100%.1f%%|r | 视口：|cffffd100%s|r"
L["MSG_LAYOUT_APPLIED"] = "布局应用成功。"

-- Slash Command System Messages
L["MSG_LOADED"] = "v%s 已加载！输入 |cffffcc00/offhand|r 进行配置。"
L["MSG_FIRST_RUN"] = "初次使用 Offhand？输入 |cff00ff00/offhand wizard|r 即可进行一键自动设置与校准。"
L["MSG_LAYOUT_ERROR"] = "布局错误：%s"
L["MSG_AR_16_9"] = "宽高比已锁定为 16:9（标准宽屏）。"
L["MSG_AR_21_9"] = "宽高比已锁定为 21:9（超宽屏）。"
L["MSG_AR_FILL"] = "使用已配置的游戏高度。可通过 /offhand height <5-100 百分比> 调整。"
L["MSG_AR_CUSTOM"] = "自定义宽高比已设为 %.3f:1。"
L["MSG_HUD_SET"] = "全局界面大小已设为 Mainhand 显示器的 %.0f%%。"
L["MSG_HUD_CURRENT"] = "当前全局界面尺寸倍数：%.2f（默认 0.70）。用法：/offhand hud <25-125 百分比>"
L["MSG_CHAT_DECK"] = "聊天栏已停靠至副屏指挥区（底部仓位）。"
L["MSG_CHAT_GAME"] = "聊天栏已锁定至 Mainhand 游戏显示器（左下角）。"
L["MSG_CHAT_TOGGLED"] = "聊天栏位置已切换为：%s。"
L["MSG_SEAM_SET"] = "分界与副屏指挥区宽度已设为 %.1f%%。"
L["MSG_BOTTOM_SET"] = "游戏底部内嵌偏移已设为 %.0f 像素。"
L["MSG_HEIGHT_SET"] = "游戏高度已设为画布的 %.0f%%（用于填充模式）。"
L["MSG_DIAG_VIEWPORT"] = "视口边界检测：%s。"
L["MSG_DIAG_PASS"] = "通过"
L["MSG_DIAG_MISMATCH"] = "不匹配"
L["MSG_DIAG_GAME_PIX"] = "游戏像素：%dx%d 位于 (%d, %d)，全局界面缩放 %.3f。"
L["MSG_DIAG_FULL"] = "诊断信息：物理=%dx%d | 屏幕=%dx%d | 有效缩放=%.3f | 副屏宽=%d (%.1f%%) | 游戏区=%dx%d"
L["MSG_TOGGLE_ON"] = "|cff00ff00已启用|r"
L["MSG_TOGGLE_OFF"] = "|cffff3333已禁用|r"
L["MSG_TOGGLED"] = "Offhand 现已 %s。"
L["MSG_DEBUG_ON"] = "|cff00ff00开|r"
L["MSG_DEBUG_OFF"] = "|cffff3333关|r"
L["MSG_DEBUG_TOGGLED"] = "调试模式 %s。"
L["MSG_SPAN_GUIDE"] = "请使用内置的 Offhand.exe 工具将 WoW 拉伸至跨越两台显示器。"
L["MSG_STATUS"] = "状态：%s。输入 |cffffcc00/offhand|r 打开选项，或输入 |cff00ff00/offhand wizard|r 打开自动配置。"
L["MSG_PANEL_LAYOUT_ERROR"] = "面板布局错误：%s"
L["MSG_MAP_LAYOUT_ERROR"] = "地图布局错误：%s"

-- Profile Management
L["TAB_PROFILES"] = "配置"
L["PROFILES_CURRENT_LABEL"] = "当前配置："
L["PROFILES_LIST_TITLE"] = "已存配置"
L["PROFILES_BTN_CREATE"] = "创建"
L["PROFILES_BTN_LOAD"] = "加载"
L["PROFILES_BTN_COPY"] = "复制自"
L["PROFILES_BTN_DELETE"] = "删除"
L["PROFILES_BTN_RESET"] = "重置当前"
L["PROFILES_CREATE_TIP_TITLE"] = "创建配置"
L["PROFILES_CREATE_TIP_DESC"] = "使用指定名称创建新配置。"
L["MSG_PROFILE_CREATED"] = "已创建配置：%s"
L["MSG_PROFILE_LOADED"] = "已加载配置：%s"
L["MSG_PROFILE_DELETED"] = "已删除配置：%s"
L["MSG_PROFILE_RESET"] = "配置已恢复默认。"
L["MSG_PROFILE_COPIED"] = "已从配置复制设置：%s"
L["PROFILES_WARN_DELETE_ACTIVE"] = "无法删除当前正在使用的配置。"
L["PROFILES_WARN_DEFAULT"] = "无法删除默认（Default）配置。"
L["PROFILES_WARN_EMPTY_NAME"] = "配置名称不能为空。"
L["PROFILES_WARN_EXISTS"] = "已存在同名配置。"
-- New Setup UI Strings

L["POPUP_COMPANION_WARNING_TEXT"] = "|cffd0d0d0Offhand 已启用，但当前窗口未根据您的物理显示器布局与分辨率达到最佳跨屏状态。|r\n\n为获得流畅的无边框全屏体验，并避免每次启动游戏时都要手动拉伸窗口边缘，强烈建议配合使用 Offhand Companion 配套程序。可从下方 GitHub 安全下载：\n\n（或者，如果您更喜欢手动将窗口拖拽拉伸至两台显示器，可点击“忽略”以永久关闭此提醒）。"
L["POPUP_WELCOME_WARNING_TEXT"] = "|cffffd100欢迎使用 Offhand！|r\n\n若要使用此双显示器界面，World of Warcraft 窗口必须跨越两台屏幕。目前游戏仅在单个屏幕上运行。\n\n我们强烈推荐使用 |cff00ff00Offhand Companion 伴侣程序|r（Windows .exe）实现像素级精准、无边框的跨屏显示。\n\n当游戏成功跨屏后，点击“启动向导”开始校准界面。"
L["POPUP_BTN_GET_APP"] = "获取程序（复制链接）"
L["POPUP_BTN_LAUNCH_WIZARD"] = "启动向导"
L["POPUP_BTN_IGNORE"] = "忽略"
L["WIZARD_WELCOME_TEXT"] = "|cffffd100欢迎使用 Offhand！|r 您的 World of Warcraft 窗口已成功跨越显示器。\n\nOffhand 将精准分割界面：主显示器呈现 3D 游戏画面，副显示器则提供整洁的“画布”以供查看地图、背包与面板。\n\n点击下方的 |cff00ff00一键自动设置|r 即可立即校准。"

-- Newly Added Fields
L["ENABLE_OFFHAND"] = "启用 Offhand 多显示器模式"
L["SHOW_MINIMAP_ICON"] = "显示小地图图标"
L["STACKED_LAYOUT"] = "显示器上下排列"
L["PRESET_STACK_BOTTOM"] = "上下排列：游戏在下方"
L["PRESET_STACK_TOP"] = "上下排列：游戏在上方"
L["MAP_SETTINGS"] = "世界地图缩放与导航"
L["MAP_SCALE"] = "地图缩放（原始大小百分比）"
L["MAP_KEEP_OPEN"] = "移动时保持世界地图打开"
L["PANELS_INDEPENDENT"] = "独立保持工作区窗口打开"
L["RELOAD_PERSIST"] = "重载及切换区域后恢复窗口"
L["REDIRECT_POPUPS"] = "将对话框移离显示器边界"
L["OBS_SETUP"] = "OBS 采集设置"
L["OBS_HELP"] = "使用两个游戏采集源并添加裁剪滤镜。按这些像素值裁剪，然后对齐来源。"
L["OBS_GAME"] = "游戏画面裁剪"
L["OBS_WORKSPACE"] = "工作区裁剪"
L["MAP_SCALE_HELP"] = "按住 Ctrl 并在地图上滚动鼠标滚轮，在显示器范围内调整大小。"
L["PANELS_INDEPENDENT_HELP"] = "同时打开背包、角色、法术书和地图。"
L["COMPAT_LEATRIX"] = "地图由 Leatrix Maps 管理。"
L["COMPAT_BOTH"] = "背包及小地图插件自行管理窗口。"
L["COMPAT_BAGS"] = "背包插件自行管理背包窗口。"
L["COMPAT_MINIMAP"] = "小地图插件自行管理小地图。"
L["COMPAT_AUTO"] = "自动检测兼容的背包及小地图插件。"
L["MAP_KEEP_OPEN_DESC"] = "Allows navigating with map open. (Compatible with Leatrix)"
L["PANELS_INDEPENDENT_DESC"] = "Allows opening bags, character pane, spellbook & map simultaneously."
