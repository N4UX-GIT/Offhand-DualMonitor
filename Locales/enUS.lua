...
L["HELP_SETUP_BODY"] = "1. Install Offhand and Companion v2.0.0 or newer.\n2. Set WoW to standard Windowed mode (not Windowed Fullscreen).\n3. Start Companion before WoW, select the displays, enable automatic spanning, then launch WoW or click Span WoW Now.\n4. Run /offhand wizard and finish all four calibration steps.\n5. On Forever, configure protected HUD frames in Blizzard Edit Mode and save/select the Offhand layout.\n6. Exit WoW normally once, leave Companion running, then relaunch to verify the cold-start layout."
L["HELP_COMPANION_TITLE"] = "Why Forever needs Companion v2.0+"
L["HELP_COMPANION_BODY"] = "A WoW addon cannot resize or borderlessly span the Windows game client. Forever beta can also write Offhand SavedVariables without reliably restoring them on the next launch. Companion spans the selected displays, removes window borders, waits until WoW closes, snapshots the newest valid Offhand state, and restores it before the next cold launch. Manual spanning remains possible on other clients, but reliable Forever cold-launch recovery currently depends on Companion v2.0.0 or newer."
L["HELP_PANELS_TITLE"] = "Use the Offhand Monitor"
L["HELP_PANELS_BODY"] = "Drag supported workspace windows such as the map, character sheet, chat and bags by their headers. Their position and open/closed state are restored after reload and cold launch. Enable independent panels to keep several open together. Hold Ctrl and use the mouse wheel over the world map to resize it. Blizzard-protected action bars and combat frames must be positioned through Edit Mode instead."
L["HELP_RECOVERY_TITLE"] = "Recover windows or return to one monitor"
L["HELP_RECOVERY_BODY"] = "Use Gather Off-Screen UI to rescue ordinary movable windows. For protected Forever HUD elements, use Blizzard Edit Mode. If Edit Mode controls are unavailable, click Restore Window in Companion, enter Edit Mode, then Span WoW Now; Offhand will keep the options frame in the game viewport. To return to one monitor, disable Offhand and use Restore Window (Ctrl+Alt+R). Always exit WoW normally before closing Companion so the latest Forever state can be captured."
L["HELP_EDIT_MODE_TITLE"] = "Position the Forever HUD with Edit Mode"
L["HELP_EDIT_MODE_BODY"] = "Forever keeps Blizzard Edit Mode in control of action bars, player/target, stance, pet, party and raid frames to prevent protected-UI errors. Outside combat, open Edit Mode, move those frames onto the Mainhand Monitor, create or save the layout with the exact name Offhand, and select it. Offhand keeps the Edit Mode options frame inside the game viewport but does not move protected HUD frames or switch layouts automatically. Select your normal layout before returning to one monitor."
