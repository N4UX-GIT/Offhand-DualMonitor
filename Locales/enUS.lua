...
L["HELP_SETUP_BODY"] = "1. Install Offhand and enable it for your character.\n2. Set WoW to standard Windowed mode, not Windowed (Fullscreen).\n3. Install Companion v2.0.0 or newer and start it before WoW.\n4. Select the displays, enable automatic spanning, then launch WoW or click Span WoW Now.\n5. Open /oh, launch the Wizard, and complete all four calibration steps.\n6. On Forever, open Blizzard Edit Mode, select/create the Offhand layout, position protected HUD frames on Mainhand, and save.\n7. Arrange the map, character frame, backpack and chat on the Offhand workspace.\n8. Exit WoW normally with Companion running, then relaunch to verify a cold start."
L["HELP_COMPANION_TITLE"] = "Why Forever needs Companion v2.0+"
L["HELP_COMPANION_BODY"] = "A WoW addon cannot resize or borderlessly span the Windows game client. The current Forever beta can also write addon SavedVariables without reliably restoring them after a full client restart. That is why a layout may survive /reload but return to default anchors after a cold launch. Companion solves both problems: it spans the selected displays before Offhand lays out the UI and, while WoW is closed, refreshes only Offhand's recovery bridge with the newest valid state. It does not inspect game memory or automate gameplay."
